...
export function CropOverlay({ edits, onEditChange, containerWidth, containerHeight }: CropOverlayProps) {
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (dragType === "move") {
        const newX = Math.max(0, Math.min(1 - initialCrop.width, initialCrop.x + deltaX))
        const newY = Math.max(0, Math.min(1 - initialCrop.height, initialCrop.y + deltaY))
        ...
        const newWidth = Math.max(0.1, Math.min(1 - initialCrop.x, initialCrop.width + deltaX))
        const newHeight = Math.max(0.1, Math.min(1 - initialCrop.y, initialCrop.height + deltaY))
    ...
    [isDragging, dragType, dragStart, initialCrop, containerWidth, containerHeight, edits.crop, onEditChange],
  )
